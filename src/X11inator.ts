export function do_x11inator() {
    ...
    x11.createClient(function(err: Error | undefined, display: any) {
        if (err) throw err

        var min = display.min_keycode, // TODO: explain min_keycode
            max = display.max_keycode, // TODO: explain max_keycode
            chr2Data = [] as Array<any>, // allow us to find a char by the charcode
            key2Data = [] as Array<any> // associate chars to a keycode
        ...
        X.GetKeyboardMapping(min, max - min, function(err: any, list: any) {
            for (var i = 0; i < list.length; ++i) {
                var name = (key2Data[i + min] = [] as Array<any>)
                var sublist = list[i]
        ...
        X.on("event", function(ev: any) {
            if (ev.type == 2) {
                ...
                if (key) {
                    ...
                    console.log(
                        ">> keyCode " + ev.keycode + " was not recognized."
                    )
            }
        })
    })
